...
class MyApp:
    def __init__(self):
        ...
        label6=tk.Label(root,text="Cpassword:").grid(row=5,column=0)

        entry6=tk.Entry(root,show="*",textvariable=self.v5).grid(row=5,column=1)

        button1=tk.Button(root,text="Submit",command=lambda: self.validate()).grid(row=6,column=1)
    ...
    def validate(self):
        ...
        if a5=='':
            ...
            if res:
                ...
                str1="INSERT INTO `student` (`name`, `email`, `phone`,`gender`, `password`) VALUES ('"+a1+"', '"+a2+"', '"+a3+"','"+a4+"', '"+a5+"')";
                try:
                    cur.execute(str1)
                    conn.commit()
